        ...
        {gastos.length === 0 ? (
          ...
          <>
            ...
            <div className="lg:hidden flex flex-col gap-3 p-3">
              {gastos.map((g) => (
                <div key={g.id} className="rounded-xl border border-[var(--border)] p-3.5 flex flex-col gap-2">
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      <div className="muted text-[12px]">{fecha(g.fecha)} · {g.categoria}{g.tipo === "MIOS" && " · mío"}</div>
                      <div className="font-semibold text-[15px] truncate">{g.proveedor ?? g.concepto}</div>
                      {g.proveedor && <div className="muted text-[13px] truncate">{g.concepto}</div>}
                    </div>
                  ...
                  {g.archivo ? (
                    <div className="text-[12px] text-[var(--tone-green)]">✓ {g.archivo}</div>
                  ) : (
                    <Link
                      href={`/gastos/${g.id}/foto`}
                      className="text-center rounded-lg bg-[var(--brand-teal-dark)] text-white text-[13px] font-semibold py-2"
                    >
                      📷 Subir foto del justificante
                    </Link>
                  )}
                    ...
                    <tr key={g.id}>
                      <td className="font-medium whitespace-nowrap">
                        {g.archivo ? (
                          ...
                          <Link href={`/gastos/${g.id}/foto`} className="text-[var(--brand-teal-dark)] hover:underline" title="Falta la foto del justificante: pulsa para subirla">
